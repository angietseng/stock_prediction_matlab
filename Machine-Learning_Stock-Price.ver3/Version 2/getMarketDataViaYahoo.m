function data = getMarketDataViaYahoo(symbol, startdate, enddate, interval)
    ...
    if(nargin() == 1) % input symbol only
        startdate = posixtime(datetime('1-Jan-2021'));
        enddate = posixtime(datetime()); % now
        interval = '1d';
    elseif (nargin() == 2) % input symbol and start date 
        startdate = posixtime(datetime(startdate));
        enddate = posixtime(datetime()); % now
        interval = '1d';
    elseif (nargin() == 3) % input symbol, startdate, and enddate
        startdate = posixtime(datetime(startdate));
    ...
    elseif(nargin() == 4)
        ...
        data = [];
        return; % why we need return here %
    end
